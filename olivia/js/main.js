var App = new (function () {
  ...
  this.getForecast = function getForecast() {
    ...
    elScript.src =
      ...
      ",2021-10-22T15:30:00+0300?units=si&exclude=minutely,hourly,flags&callback=App.renderForecast";
  ...
  this.renderForecast = function renderForecast(response) {
    ...
    if (response && response.currently) {
      var skycon = new Skycons({ color: "#ff7200", resizeClear: true });
      skycon.add(
        "skycon",
  ...
  this.initGoogleMaps = function initGoogleMaps() {
    ...
    var partyInfoWindow = new google.maps.InfoWindow({
      content:
        '<div id="content"><h4>El cumple de Olivia</h4><p>Los esperamos el <strong>Viernes 22/10</strong> a las <strong>15:30</strong></p></div>',
    });
